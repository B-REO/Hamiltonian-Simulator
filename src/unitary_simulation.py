""" This is the simulator for a unitary Hamiltonian on a single qubit.

In this program, you get a random unitary Hamiltonian and initial state 

and simulate time-development of Hamiltonian on a single qubit.
"""
...
def unitary_hamiltonian_maker(order):
    ''' In this part, you get a random unitary Hamiltonian.

    Args:
        order (int): parameter to judge unitarity

    Returns:
        np.array(2*2) (complex): unitary Hamiltonian

    Examples:
        Usage

        >>> Ham_time.unitary_maker(6)
        array([[ 0.61457468+0.23631462j,  0.97558406+0.30314112j],
               [-0.9273539 -0.42857152j,  0.61457468+0.23631462j]])
    '''
    mat = np.array([[0.8 + 0.j, 3 + 0.j], [0.1 + 0.j, 0.5 + 0.j]])
    mat[0][0] = np.random.rand() + np.random.rand()*1.j
    mat[1][1] = mat[0][0]
    while (np.abs(mat[0][1]*np.conjugate(mat[0][1]) + mat[0][0]*np.conjugate(mat[0][0]) - 1.0) > 1.0e-8):
        mat[0][1] = np.random.rand() + np.random.rand()*1.j
    mat[1][0] = -np.conjugate(mat[0][1])/np.conjugate(mat[0][0])*mat[0][0]
    return mat
...
def exp_maker(mat, time_int):
    ''' In this part, you get time development of unitary Hamiltonian.

    Args:
        mat (np.array(2*2) (complex)): unitary Hamiltonian
        time_int (float): time interval for time-development

    Returns:
        np.array(2*2) (complex): time development of unitary Hamiltonian
